...
async function authModeloCompleto(req, res, next) {
  try {
    ...
    // 2️⃣ Verificar se dados existem usando modelo_id
    const result = await db.query(
      "SELECT 1 FROM modelos_dados WHERE modelo_id = $1",
      [modelo_id]
    );
    ...
    console.error("Erro authModeloCompleto:", err);
...
function authCliente(req, res, next) {
  ...
  try {
    ...
    if (!decoded || !decoded.id || decoded.role !== "cliente") {
...
function authModelo(req, res, next) {
  ...
  try {
    ...
    if (!decoded || !decoded.id || decoded.role !== "modelo") {
