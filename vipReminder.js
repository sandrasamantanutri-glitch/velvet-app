...
async function processarAvisos() {
  ...
  console.log("🔎 TESTANDO cliente_id 2367");
  ...
  try {
    ...
    if (result.rows.length === 0) {
      console.log("❌ Nenhuma VIP ativa encontrada para cliente 2367");
      process.exit();
    }

    const row = result.rows[0];

    console.log("📧 Email encontrado:", row.email);

    await enviarEmail(
      row.email,
      "TESTE VIP CLIENTE 2367",
      `<h1>Teste funcionando para cliente 2367</h1>`
    );
