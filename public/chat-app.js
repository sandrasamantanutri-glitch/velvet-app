...
function criarItemConversa(c) {
  ...
  // ⏱ tempo
  const lastTime = c.ultima_msg_ts || c.ultima_msg_modelo_ts || c.ultima_msg_cliente_ts;
  div.dataset.lastTime = lastTime
    ? new Date(lastTime).getTime()
    : 0;

  const nome = c.nome || c.username;
  ...
  div.innerHTML = `
    ...
    <div class="info">
      ...
      <div class="preview">
        ${c.ultima_msg_texto || "Toque para abrir o chat"}
      </div>
    </div>
...
function abrirChat(c) {
  ...
  alert("PASSO 4 → abrir sala de chat");
}
