...
// ========== 13b. TRANSAÇÕES CARTÃO (Stripe, agrupadas por dia de compra + tipo) ==========
...
router.get("/transacoes-agency-cartao", async (req, res) => {
  try {
    ...
    // Stripe — inclui pago e chargeback, filtra por mês de compra
    let where = "m.verificada = true AND m.ativo = true AND t.gateway = 'stripe' AND t.status IN ('pago','chargeback')";
    const params = [];
    let pIdx = 1;
    if (modelo_id) { where += ` AND t.modelo_id = $${pIdx}`; params.push(modelo_id); pIdx++; }
    if (m) {
    ...
    const { rows } = await db.query(
      `SELECT m.id, COALESCE(m.nome_exibicao, m.nome) AS nome,
       ...
       INNER JOIN messages msg ON msg.modelo_id = m.id
         ...
         AND msg.tipo IN ('conteudo_ppv_mass', 'conteudo_ppv', 'ppv')
         AND msg.preco > 0
       GROUP BY m.id, m.nome_exibicao, m.nome
       ORDER BY total_ppvs DESC`
    );
    res.json(rows);
  } catch (err) {
    console.error("Erro ppvs-admin modelos:", err);
    res.status(500).json({ erro: "Erro interno" });
  }
});
...
router.get("/ppvs-admin", async (req, res) => {
  try {
    ...
    console.error("Erro ppvs-admin:", err);
    res.status(500).json({ erro: "Erro interno" });
  }
});

router.get("/ppvs-admin/:id", async (req, res) => {
...
router.get("/saldo-modelo/:id", authAdmin, async (req, res) => {
  try {
    ...
    const [ganhosRes, pagosRes, saquesRes] = await Promise.all([
      db.query(`
        SELECT
          ...
          COALESCE(SUM(valor_modelo) FILTER (
            WHERE gateway IS DISTINCT FROM 'stripe'
      ...
      db.query(`
        SELECT
          COALESCE(SUM(valor + COALESCE(taxa_saque,0)) FILTER (WHERE status IN ('pago','pendente')), 0) AS comprometidos,
          COALESCE(SUM(valor + COALESCE(taxa_saque,0)) FILTER (WHERE status = 'pendente'), 0) AS pendentes
    ...
    const ganhos_liberados_mes = Number(ganhosRes.rows[0].ganhos_liberados_mes);
    const pagos               = Number(pagosRes.rows[0].pagos);
    const comprometidos       = Number(saquesRes.rows[0].comprometidos);
    const saques_pendentes    = Number(saquesRes.rows[0].pendentes);
    const saldo_disponivel    = Math.max(0, ganhos_liberados - pagos - comprometidos);
    res.json({ ganhos_geral, ganhos_liberados_mes, saques_pendentes, saldo_disponivel });
  } catch (err) {
